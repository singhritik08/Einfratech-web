...
function Header() {
  ...
  const [isDropdown1Clicked, setIsDropdown1Clicked] = useState(false); // Track click state
  const [isDropdown2Clicked, setIsDropdown2Clicked] = useState(false); // Track click state
  ...
  const handleDashboardOptionClick = (option) => {
    ...
    setIsDropdown1Clicked(false); // Reset click state
  ...
  const handleContactOptionClick = (option) => {
    ...
    setIsDropdown2Clicked(false); // Reset click state
    setIsMobileMenuOpen(false);
  ...
  // Dropdown 1 (Dashboard) handlers
  const handleDropdown1Click = () => {
    setIsDropdown1Open(!isDropdown1Open);
    setIsDropdown1Clicked(!isDropdown1Open); // Toggle clicked state
  };
  ...
  const handleDropdown1MouseLeave = () => {
    if (!isDropdown1Clicked) setIsDropdown1Open(false); // Only close if not clicked
  };

  // Dropdown 2 (Contact) handlers
  const handleDropdown2Click = () => {
    setIsDropdown2Open(!isDropdown2Open);
    setIsDropdown2Clicked(!isDropdown2Open); // Toggle clicked state
  };
  ...
  const handleDropdown2MouseLeave = () => {
    if (!isDropdown2Clicked) setIsDropdown2Open(false); // Only close if not clicked
  };

  return (
    <nav className='bg-[#F9FAFB] h-16 px-4 sm:px-8 py-4 max-w-screen-2xl flex justify-between items-center'>
      {/* Logo Section */}
      <div 
      ...
      <div className='hidden md:flex items-center space-x-4'>
        ...
        >
          <button
            className="text-[#000000] flex p-1.5 bg-white rounded-md items-center hover:bg-gray-100"
        ...
        >
          <button
            className="text-[#000000] flex p-1.5 items-center hover:text-[#0D6EFD]"
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="absolute top-16 left-0 w-full bg-[#F9FAFB] md:hidden shadow-lg z-20">
          <div className="flex flex-col p-4 space-y-3">
            {/* Dashboard Dropdown */}
            <div className="relative">
              <button
                className="text-[#000000] flex p-2 bg-white rounded-md w-full justify-between hover:bg-gray-100"
                onClick={() => setIsDropdown1Open(!isDropdown1Open)}
              >
                <h1 className="text-sm">{selectedDashboardOption}</h1>
                <FontAwesomeIcon icon={faCaretDown} className="text-sm" />
              </button>
              {isDropdown1Open && (
            ...
            {/* Static Links */}
            <div 
              className='py-1 text-sm hover:text-[#0D6EFD] cursor-pointer' 
              onClick={() => handleStaticLinkClick('/customer')}
            >
              Customers
            </div>
            ...
            <div className="relative">
              <button
                className="text-[#000000] flex p-2 w-full justify-between hover:text-[#0D6EFD]"
                onClick={() => setIsDropdown2Open(!isDropdown2Open)}
              >
                <h1 className="text-sm">{selectedContactOption}</h1>
                <FontAwesomeIcon icon={faCaretDown} className="text-sm" />
              </button>
              {isDropdown2Open && (
                <div className="bg-white text-black shadow-md rounded-md mt-2 w-full">
